...
- **In rare instances the extension may display the registration page multiple times.** For already registered users, upon browser updates, the extension may display the user registration page again. If the user registers again, a second Firebase user will be created and data will be collected from that moment on under that Firebase ID. However, datasets for the same user under different Firebase IDs can be combined in the post-processing stages.
