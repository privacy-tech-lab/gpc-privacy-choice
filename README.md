...
2. Create a Firebase Cloud Firestore project. [Here](https://firebase.google.com/docs/firestore/quickstart) are detailed instructions on how to set up a Firebase server for data collection. We have already added the required dependencies, client libraries, and initialized an instance of Cloud Firestore. Before proceeding, make sure that you have created a Firebase account and a Cloud Firestore database, selected **Test Mode** as the starting mode for your Cloud Firestore Security Rules, and installed Firebase to your local machine through the command `npm install firebase`.
