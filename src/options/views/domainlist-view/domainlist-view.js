...
function addEventListeners() {
  ...
  document.addEventListener('click', event => {
    if (event.target.id=='allow-all-btn'){
        chrome.storage.local.get(["DOMAINS", "ENABLED"], function (result) {
            if (allOn(result.DOMAINS) === false && allOff(result.DOMAINS) !== true) {
              chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All" , location: "N/A", origin: "Options page", setting: "GPC Signal", prevSetting: "Personalized domain list" , newSetting: "Allow tracking", universalSetting: true})
            }
            if (allOn(result.DOMAINS) === true) {
              chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All" , location: "N/A", origin: "Options page", setting: "GPC Signal", prevSetting: "Don't Allow Tracking" , newSetting: "Allow tracking", universalSetting: true})
            }
      ...
      chrome.storage.local.set({DOMAINLIST_ENABLED: false});
        ...
        chrome.storage.local.get(["DOMAINS"], function (result) {
            if (allOff(result.DOMAINS) === false && allOn(result.DOMAINS) !== true) {
              chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All", location: "N/A", origin: "Options page", setting: "GPC signal", prevSetting: "Personalized domain list" , newSetting: "Don't allow tracking", universalSetting: true})
            }
            if (allOff(result.DOMAINS) === true) {
              chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All" , location: "N/A", origin: "Options page", setting: "GPC signal", prevSetting: "Allow Tracking" , newSetting: "Don't allow tracking", universalSetting: true})
            }
            let new_domains = result.DOMAINS;
            for (let d in new_domains){
                new_domains[d] = true;
    ...
    if(event.target.id=='apply-all-off-btn'){
      chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All" , location: "N/A", origin: "Options page", setting: "Universal Setting", prevSetting: "On" , newSetting: "Off", universalSetting: false})
    ...
    if(event.target.id=='toggle_all_on'){
      chrome.storage.local.get(["DOMAINS", "APPLY_ALL"], function (result) {
        if (allOff(result.DOMAINS) === false && allOn(result.DOMAINS) !== true) {
          chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All existing domains" , location: "N/A", origin: "Options page", setting: "GPC signal", prevSetting: "Personalized domain list" , newSetting: "Don't allow tracking", universalSetting: result.APPLY_ALL })
        }
        if (allOff(result.DOMAINS) === true) {
          chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All existing domains" , location: "N/A", origin: "Options page", setting: "GPC signal", prevSetting: "Allow Tracking" , newSetting: "Don't allow tracking", universalSetting: result.APPLY_ALL})
        }
        let new_domains = result.DOMAINS;
        for (let d in new_domains){
            new_domains[d] = true;
    ...
    if(event.target.id=='toggle_all_off'){
      chrome.storage.local.get(["DOMAINS", "APPLY_ALL"], function (result) {
        if (allOn(result.DOMAINS) === false && allOff(result.DOMAINS) !== true) {
          chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All existing domains" , location: "N/A", origin: "Options page", setting: "GPC Signal", prevSetting: "Personalized domain list" , newSetting: "Allow tracking", universalSetting: result.APPLY_ALL})
        }
        if (allOn(result.DOMAINS) === true) {
          chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All existing domains" , location: "N/A", origin: "Options page", setting: "GPC Signal", prevSetting: "Don't Allow Tracking" , newSetting: "Allow tracking", universalSetting: result.APPLY_ALL})
        }
        let new_domains = result.DOMAINS;
        for (let d in new_domains){
            new_domains[d] = false;
    ...
    if(event.target.id=='delete_all_domainlist'){
        ...
        if (confirm(delete_prompt)) {
          chrome.storage.local.get(["APPLY_ALL"], function (result) {
            chrome.runtime.sendMessage({greeting:"INTERACTION", domain: "All existing domains" , location: "N/A", origin: "Options page", setting: "Delete domain", prevSetting: None , newSetting: None, universalSetting: result.APPLY_ALL})
          })
...
NOTE: It will be automatically added back to the list when the domain is requested again.`
      if (confirm(delete_prompt)) {
        chrome.storage.local.get(["APPLY_ALL"], function (result) {
          chrome.runtime.sendMessage({greeting:"INTERACTION", domain: domain , location: "N/A", origin: "Options page", setting: "Delete domain", prevSetting: None , newSetting: None, universalSetting: result.APPLY_ALL})
        })
